...
vec3 colormap(vec3 direction){vec2 xy=normalize(direction.yz);float hue=atan2(xy.x,xy.y)/ 3.14159 / 2.0;return hsv2rgb(vec3(hue,1.0,1.0));}`};WebGLSpins.renderModes={};WebGLSpins.defaultOptions={};WebGLSpins.defaultOptions.verticalFieldOfView=45;WebGLSpins.defaultOptions.allowCameraMovement=true;WebGLSpins.defaultOptions.colormapImplementation=WebGLSpins.colormapImplementations['red'];WebGLSpins.defaultOptions.cameraLocation=[0,0,1];WebGLSpins.defaultOptions.centerLocation=[0,0,0];WebGLSpins.defaultOptions.upVector=[0,1,0];WebGLSpins.defaultOptions.backgroundColor=[0,0,0];WebGLSpins.prototype.updateOptions=function(options){var changedOptions=[];for(var option in options){if(this._options.hasOwnProperty(option)){if(this._options[option]!==options[option]){this._options[option]=options[option];changedOptions.push(option);}}else{console.warn("WebGLSpins does not recognize option '"+option+"'.");}}
if(changedOptions.length==0){return;}
if(changedOptions.indexOf('renderMode')!=-1){this._renderer.cleanup();this._renderer=new this._options.renderMode(this);}else{this._renderer.optionsHaveChanged(changedOptions);}
...
this._renderer.updateSpins(this._instancePositionArray,this._instanceDirectionArray);this.draw();};WebGLSpins.prototype.draw=function(){var gl=this._gl;var width=this._canvas.clientWidth;var height=this._canvas.clientHeight;this._canvas.width=width;this._canvas.height=height;gl.viewport(0,0,gl.drawingBufferWidth,gl.drawingBufferHeight);gl.clearColor(this._options.backgroundColor[0],this._options.backgroundColor[1],this._options.backgroundColor[2],1.0);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);this._renderer.draw();};WebGLSpins.prototype._mergeOptions=function(options,defaultOptions){this._options={};for(var option in defaultOptions){this._options[option]=defaultOptions[option];}
...
this._gl=gl;gl.enable(gl.DEPTH_TEST);gl.depthFunc(gl.LEQUAL);gl.enable(gl.CULL_FACE);this._renderer.initGLContext();this._gl_initialized=true;};WebGLSpins.prototype._handleMouseDown=function(event){if(!this._options.allowCameraMovement){return;}
...
if(webglspins._instancePositionArray!=null&&webglspins._instanceDirectionArray!=null){this.updateSpins(webglspins._instancePositionArray,webglspins._instanceDirectionArray)}};WebGLSpins.renderModes.ARROWS=WebGLSpins._ArrowRenderer;WebGLSpins.defaultOptions.coneHeight=0.6;WebGLSpins.defaultOptions.coneRadius=0.25;WebGLSpins.defaultOptions.cylinderHeight=0.7;WebGLSpins.defaultOptions.cylinderRadius=0.125;WebGLSpins.defaultOptions.levelOfDetail=20;WebGLSpins.defaultOptions.renderMode=WebGLSpins.renderModes.ARROWS;WebGLSpins._ArrowRenderer.prototype.optionsHaveChanged=function(changedOptions){var arrayContainsAny=function(array,values){for(var i=0;i<values.length;i++){if(~array.indexOf(values[i])){return true;}}
...
vec3 colormap(vec3 direction);void main(void){vfColor=colormap(normalize(ivInstanceDirection));mat3 instanceMatrix=matrixFromDirection(ivInstanceDirection);vfNormal=(uModelviewMatrix*vec4(instanceMatrix*ivNormal,0.0)).xyz;vfPosition=(uModelviewMatrix*vec4(instanceMatrix*ivPosition+ivInstanceOffset,1.0)).xyz;gl_Position=uProjectionMatrix*vec4(vfPosition,1.0);}`+this._options.colormapImplementation);gl.compileShader(vertexShader);if(!gl.getShaderParameter(vertexShader,gl.COMPILE_STATUS)){console.error("vertex shader info log:\n"+gl.getShaderInfoLog(vertexShader));return;}
...
if(webglspins._instancePositionArray!=null&&webglspins._instanceDirectionArray!=null){this.updateSpins(webglspins._instancePositionArray,webglspins._instanceDirectionArray)}};WebGLSpins.renderModes.SURFACE=WebGLSpins._SurfaceRenderer;WebGLSpins.defaultOptions.surfaceIndices=[];WebGLSpins._SurfaceRenderer.prototype.optionsHaveChanged=function(changedOptions){var arrayContainsAny=function(array,values){for(var i=0;i<values.length;i++){if(~array.indexOf(values[i])){return true;}}
return false;};if(arrayContainsAny(changedOptions,['surfaceIndices'])){this._updateSurfaceIndices();}
if(arrayContainsAny(changedOptions,['colormapImplementation'])){this._updateShaderProgram();}};WebGLSpins._SurfaceRenderer.prototype.updateSpins=function(instancePositionArray,instanceDirectionArray){var gl=this._webglspins._gl;this._numInstances=instancePositionArray.length/3;gl.bindBuffer(gl.ARRAY_BUFFER,this._instancePositionVbo);gl.bufferData(gl.ARRAY_BUFFER,instancePositionArray,gl.STREAM_DRAW);gl.bindBuffer(gl.ARRAY_BUFFER,this._instanceDirectionVbo);gl.bufferData(gl.ARRAY_BUFFER,instanceDirectionArray,gl.STREAM_DRAW);};WebGLSpins._SurfaceRenderer.prototype.draw=function(){var gl=this._webglspins._gl;if(this._numIndices<3){return;}
gl.bindBuffer(gl.ARRAY_BUFFER,this._instancePositionVbo);gl.vertexAttribPointer(0,3,gl.FLOAT,false,0,0);gl.enableVertexAttribArray(0);gl.bindBuffer(gl.ARRAY_BUFFER,this._instanceDirectionVbo);gl.vertexAttribPointer(1,3,gl.FLOAT,false,0,0);gl.enableVertexAttribArray(1);gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,this._ibo);gl.useProgram(this._program);var width=this._webglspins._canvas.clientWidth;var height=this._webglspins._canvas.clientHeight;var projectionMatrix=WebGLSpins._perspectiveProjectionMatrix(this._options.verticalFieldOfView,width / height,0.1,10000);gl.uniformMatrix4fv(gl.getUniformLocation(this._program,"uProjectionMatrix"),false,WebGLSpins._toFloat32Array(projectionMatrix));var modelviewMatrix=WebGLSpins._lookAtMatrix(this._options.cameraLocation,this._options.centerLocation,this._options.upVector);gl.uniformMatrix4fv(gl.getUniformLocation(this._program,"uModelviewMatrix"),false,WebGLSpins._toFloat32Array(modelviewMatrix));gl.disable(gl.CULL_FACE);gl.drawElements(gl.TRIANGLES,this._numIndices,gl.UNSIGNED_INT,null);gl.enable(gl.CULL_FACE);};WebGLSpins._SurfaceRenderer.prototype.initGLContext=function(){var gl=this._webglspins._gl;var oes_element_index_uint_ext=gl.getExtension("OES_element_index_uint");if(!oes_element_index_uint_ext){console.error('WebGL does not support OES_element_index_uint required by WebGLSpins');return;}
...
var fragmentShader=gl.createShader(gl.FRAGMENT_SHADER);gl.shaderSource(fragmentShader,`#version 100
precision highp float;varying vec3 vfDirection;vec3 colormap(vec3 direction);void main(void){vec3 color=colormap(normalize(vfDirection));gl_FragColor=vec4(color,1.0);}`+this._options.colormapImplementation);gl.compileShader(fragmentShader);if(!gl.getShaderParameter(fragmentShader,gl.COMPILE_STATUS)){console.error("fragment shader info log:\n"+gl.getShaderInfoLog(fragmentShader));return;}
